...
- Commands (`/privacy`): read from `business_message` only — `allowed_updates` never delivers a plain `message`. Answered ONLY to the sender when they are the owner of the connection the command arrived through, as a direct message without `business_connection_id`. The policy has one source of truth, `internal/privacy/policy.md`, embedded with `go:embed`; its version and effective date are parsed back from it, never duplicated in Go
